...
<script lang="ts" setup>
  ...
  import { INode, ILink, ITool, IDragInfo, IElement } from '../type/flow'
  ...
  function paste() {
    ...
    clipboard.forEach((node: INode) => {
      ...
      let nodePos = computeNodePos(
        mouse.position.x + dis,
        mouse.position.y + dis
      )
      newNode.x = nodePos.x
      newNode.y = nodePos.y
      dis += 20
  ...
  watch(
    ...
    (val) => {
      ...
      if (unref(currentSelect).type === 'link') {
        ...
        document
          .querySelector('#' + unref(currentSelect).id)
